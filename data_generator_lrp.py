...
class LRPDataGenerator(object):
    ...
    # =========================================================================
    # Reward-shaping gain matrix:  g_ij = max(0, d_safe(i,j) − d(i,j))
    # =========================================================================
    @staticmethod
    def _compute_gain_single(coords, lam):
        """
        For one instance, compute normalized gain matrix.
        coords: [N, 2],  lam: [N, N]
        Returns: gain_normalized [N, N] in [0, 1].
        """
        n = coords.shape[0]
        ...
        # Safe-edge-only graph (weight = dist if safe, 0 = no edge)
        safe_w = np.where(lam == 1, dist, 0.0)
        np.fill_diagonal(safe_w, 0.0)
        safe_graph = _csr_matrix(safe_w)

        # All-pairs shortest paths on safe subgraph
        safe_dist = _sp_shortest_path(safe_graph, directed=False)

        # Gain for risky edges
        gain = np.zeros((n, n), dtype=np.float32)
        risky = (lam == 0)
        np.fill_diagonal(risky, False)

        if not risky.any():
            return gain

        raw = safe_dist[risky] - dist[risky]

        # Handle inf (no safe path at all)
        finite = np.isfinite(raw)
        if finite.any():
            max_fin = max(raw[finite].max(), 1e-6)
            raw = np.where(finite, raw, max_fin * 2.0)
        else:
            raw[:] = 1.0

        gain[risky] = np.maximum(0.0, raw).astype(np.float32)
    ...
    def _compute_gain_batch(self, coords_batch, lam_batch):
        """
        Vectorized gain computation for a batch.
        coords_batch: [B, N, 2],  lam_batch: [B, N, N]
        Returns: [B, N, N]
        """
        B = coords_batch.shape[0]
        gains = np.zeros_like(lam_batch, dtype=np.float32)
        for b in range(B):
            gains[b] = self._compute_gain_single(
                coords_batch[b], lam_batch[b])
        return gains
    ...
    def _sample_instances(self, n, rng, compute_lambda=True,
        ...
        if compute_lambda:
            ...
            # Compute reward-shaping gain
            if verbose:
                print(f'  Computing gain matrix ...', end='')
            gain_parts = []
            for s in range(0, n, chunk):
                gain_parts.append(self._compute_gain_batch(
                    coords[s:s + chunk], lam[s:s + chunk]))
            gain = np.concatenate(gain_parts, axis=0)
            if verbose:
                avg_gain = gain.sum(axis=(1,2)).mean()
                print(f'  Gain done (avg sum={avg_gain:.2f}).')
        else:
            n_nodes = 1 + self._n_cust + self._n_fac
            lam  = np.ones((n, n_nodes, n_nodes), dtype=np.float32)
            gain = np.zeros((n, n_nodes, n_nodes), dtype=np.float32)
        return features, lam, gain
    ...
    def generate_fixed_dataset(self, n_instances=10000, seed=None,
        ...
        feats, lam, gain = self._sample_instances(
            n_instances, rng,
            compute_lambda=True,
        ...
        if verbose:
            ...
            print(f'  Gain      : {gain.nbytes/1e6:.1f} MB')
    ...
    def get_test_all(self):
        ...
        return self.test_features, self.test_lambda, self.test_gain
