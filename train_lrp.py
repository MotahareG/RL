...
def train():
    ...
    # ==================================================================
    # Facility Enumeration — try all 2^n_fac combinations at inference
    # ==================================================================
    n_fac     = args.get('n_fac', 8)
    fac_start = args['n_cust'] + 1
    real_cf   = args['facility_opening_cost']          # 0.00274
    n_combos  = 2 ** n_fac                             # 256 for 8 facilities

    prt.print_out(f"\n{'='*70}")
    prt.print_out(f"FACILITY ENUMERATION — testing {n_combos} combinations per instance")
    prt.print_out(f"{'='*70}")

    # ── Test 1: All stations open (quick sanity check) ────────────────────
    lambda_all_safe = np.ones_like(lambda_data)
    R_all, _, _, _, _, _, _, routes_all = sess.run(
        agent.val_summary_greedy,
        feed_dict={agent.env.input_data:     features,
                   agent.env.lambda_ph:      lambda_all_safe,
                   agent.decodeStep.dropout: 0.0})
    cost_all_open = float(np.mean(-R_all)) + n_fac * real_cf
    prt.print_out(f"  All {n_fac} stations open: avg cost = {cost_all_open:.4f}"
                  f"  (routing={float(np.mean(-R_all)):.4f} + fac={n_fac*real_cf:.4f})")

    # ── Test 2: Full enumeration ──────────────────────────────────────────
    prt.print_out(f"  Enumerating {n_combos} combos per instance ...")
    enum_t0 = time.time()

    best_costs_enum  = np.full(n_test, 1e9)
    best_config_enum = [[] for _ in range(n_test)]

    for combo in range(n_combos):
        # Build list of opened facilities for this combo
        open_facs = [fac_start + f for f in range(n_fac) if combo & (1 << f)]
        n_open    = len(open_facs)

        # Modify lambda: for each opened facility, all its edges become safe
        lam_mod = lambda_data.copy()
        for fj in open_facs:
            lam_mod[:, fj, :] = 1
            lam_mod[:, :, fj] = 1

        # Inference with modified lambda
        R_combo, _, _, _, _, _, _, _ = sess.run(
            agent.val_summary_greedy,
            feed_dict={agent.env.input_data:     features,
                       agent.env.lambda_ph:      lam_mod,
                       agent.decodeStep.dropout: 0.0})

        # Total cost per instance = routing cost + pre-opened facility cost
        combo_costs = -R_combo + n_open * real_cf    # [n_test]

        # Update best
        improved = combo_costs < best_costs_enum
        best_costs_enum[improved] = combo_costs[improved]
        for i in np.where(improved)[0]:
            best_config_enum[int(i)] = open_facs[:]

    enum_elapsed = time.time() - enum_t0

    avg_enum = float(np.mean(best_costs_enum))
    avg_base = float(np.mean(-last_R))

    prt.print_out(f"  Enumeration done in {enum_elapsed:.1f}s")
    prt.print_out(f"")
    prt.print_out(f"  {'Method':<30} {'Avg Cost':>10}")
    prt.print_out(f"  {'─'*42}")
    prt.print_out(f"  {'RL greedy (no pre-open)':<30} {avg_base:>10.4f}")
    prt.print_out(f"  {'RL + all stations open':<30} {cost_all_open:>10.4f}")
    prt.print_out(f"  {'RL + best enumeration':<30} {avg_enum:>10.4f}")
    prt.print_out(f"")

    # Per-instance details (first 5)
    prt.print_out(f"  Per-instance details (first 10):")
    prt.print_out(f"  {'Inst':>5} {'Base':>8} {'Enum':>8} {'Improv':>8} {'Opened facilities'}")
    for i in range(min(10, n_test)):
        base_i = float(-last_R[i])
        enum_i = float(best_costs_enum[i])
        imp    = base_i - enum_i
        facs_str = ', '.join(f'F{f}' for f in best_config_enum[i]) if best_config_enum[i] else '—'
        prt.print_out(f"  {i:>5} {base_i:>8.4f} {enum_i:>8.4f} {imp:>+8.4f} {facs_str}")

    prt.print_out(f"\n  ► For thesis: RL+Enum avg cost = {avg_enum:.4f}")
    prt.print_out(sep)
