...
# ==============================================================================
# Reward function  (revised model + reward shaping)
# ==============================================================================
def lrp_reward_func(
        actions,
        ...
        beta                  = 0.0):
    """
    R_train = R_base + β · B(τ)

    R_base = −( routing_cost + facility_cost )
        = − Σ_j (C_j/H)·F_j  −  Σ c_AV·d_ij·x_ijv

    B(τ)  = (1/T) · Σ_{(i,j) used} (1−λ_ij) · g̅_ij
        Bonus for actually using risky edges that yield route savings.
        g̅_ij is the normalized gain = max(0, d_safe(i,j) − d(i,j)).

    During evaluation β=0 → R = R_base (no bonus).
    """
    # ── R_base: routing + facility cost ────────────────────────────────────
    ...
    R_base = -(route_lengths + fac_cost)

    # ── Bonus B(τ): reward for using beneficial risky edges ────────────────
    if idxs_list is not None and gain_mat is not None and lambda_mat is not None:
        batch_size = tf.shape(idxs_list[0])[0]

        # idxs elements may be [B,1] — squeeze to [B]
        idxs_flat = [tf.squeeze(ix, axis=-1) if ix.shape.ndims == 2 else ix
                     for ix in idxs_list]

        # Full route: depot(0) → idxs[0] → idxs[1] → ... → idxs[T-1]
        depot = tf.zeros([batch_size], dtype=tf.int64)
        full_route = [depot] + idxs_flat

        from_nodes = tf.stack(full_route[:-1], axis=0)  # [T, B]
        to_nodes   = tf.stack(full_route[1:],  axis=0)  # [T, B]

        T_steps = tf.shape(from_nodes)[0]

        # Build gather indices [T, B, 3] → (batch_idx, from_node, to_node)
        batch_idx = tf.tile(
            tf.expand_dims(tf.range(batch_size), 0),    # [1, B]
            [T_steps, 1])                                # [T, B]
        # Cast to int64 for gather_nd compatibility
        indices = tf.stack([
            tf.cast(batch_idx, tf.int64),
            from_nodes,
            to_nodes], axis=-1)                          # [T, B, 3]

        edge_gains  = tf.gather_nd(gain_mat,   indices)  # [T, B]
        edge_lambda = tf.gather_nd(lambda_mat, indices)  # [T, B]

        # Only risky edges (λ=0) contribute, exclude self-loops
        is_risky   = 1.0 - tf.cast(edge_lambda, tf.float32)
        not_self   = tf.cast(tf.not_equal(from_nodes, to_nodes), tf.float32)
        risky_gain = is_risky * edge_gains * not_self

        # Normalize by number of real edges used
        n_edges = tf.maximum(tf.reduce_sum(not_self, axis=0), 1.0)  # [B]
        bonus   = tf.reduce_sum(risky_gain, axis=0) / n_edges       # [B]

        return R_base + beta * bonus
    else:
        return R_base
...
def train():
    ...
    args.setdefault('reward_shaping_beta', 0.25)     # β₀ for reward shaping
    args.setdefault('reward_shaping_decay_frac', 0.7)  # β → 0 in last 30% of epochs
    ...
    if use_fixed:
        prt.print_out(
            ...
            f"  β₀ (reward shaping): {args['reward_shaping_beta']}\n"
            f"  β decay fraction   : {args['reward_shaping_decay_frac']}")
    else:
        prt.print_out(
    ...
    # ── β decay schedule ──────────────────────────────────────────────────────
    beta_0     = args['reward_shaping_beta']
    decay_frac = args['reward_shaping_decay_frac']
    E_decay    = int(n_epochs * decay_frac)  # β reaches 0 at this epoch

    # ── EPOCH LOOP ────────────────────────────────────────────────────────────
    for epoch in range(n_epochs):

        # β₍ₑ₎ = β₀ · max(0, 1 − e/E_decay)
        if E_decay > 0:
            beta_e = beta_0 * max(0.0, 1.0 - epoch / E_decay)
        else:
            beta_e = 0.0
        ...
        # ── BATCH LOOP ─────────────────────────────────────────────────────────
        for batch_features, batch_lambda, batch_gain in batch_iter:

            try:
                summary = agent.run_train_step(
                    features=batch_features,
                    lambda_data=batch_lambda,
                    gain_data=batch_gain,
                    beta=beta_e)
