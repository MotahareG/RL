...
def train():
    ...
    prt.print_out(f"  {'RL beam + enum (inst 0 only)':<35} {'—':>10} {best_cost_beam_enum:>10.4f}")
    prt.print_out(f"")
    prt.print_out(f"  Instance 0 — best beam+enum config: "
                  f"{', '.join(f'F{f}' for f in best_config_beam) if best_config_beam else 'none'}")
    prt.print_out(sep)
