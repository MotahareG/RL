...
class RLAgent(object):
    ...
    def __init__(self,
        ...
        if is_train:
            ...
            # Beam-search disabled during training to save build time.
    ...
    def build_model(self, decode_type="greedy"):
        ...
        # R(τ) = R_base + β·B(τ)  (reward shaping during training)
        # β placeholder: defaults to 0 when not fed (evaluation).
        self.beta_ph = tf.placeholder_with_default(
            0.0, shape=[], name='beta')

        R = self.reward_func(
                actions,
                env.opened_facilities,
                self.args.get('facility_opening_cost', 10.0 / 3650.0),
                idxs_list=idxs,
                gain_mat=env.gain_ph,
                lambda_mat=env.lambda_ph,
                beta=self.beta_ph)
    ...
    def run_train_step(self, features=None, lambda_data=None,
                        gain_data=None, beta=0.0):
        # If features/lambda_data were passed directly (fixed-dataset training),
        # use them; otherwise pull a fresh batch from the generator.
        if features is None or lambda_data is None:
            features, lambda_data, gain_data = self.dataGen.get_train_next()

        # Fall back to the representative λ (tiled) when lambda_data is missing.
        if lambda_data is None:
            batch_size = features.shape[0]
            lambda_data = self.env.get_default_lambda_batch(batch_size)

        feed = {self.env.input_data:    features,
                self.env.lambda_ph:     lambda_data,
                self.decodeStep.dropout: self.args['dropout'],
                self.beta_ph:           beta}

        # gain_ph has a default (zeros), so only feed if we have real gain data
        if gain_data is not None:
            feed[self.env.gain_ph] = gain_data

        return self.sess.run(self.train_step, feed_dict=feed)
